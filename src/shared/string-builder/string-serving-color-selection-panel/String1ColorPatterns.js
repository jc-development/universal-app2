...
export default class String1ColorPatterns extends Component {
  constructor(props) {
    super(props);
  ...
  handleOnMouseOver(event) {
    if (document.querySelector('div.string-1.inactive') !== null)
      TweenMax.to(document.querySelector('div.string-1.inactive'), 0.5, { css: {opacity: 1} });
  }

  handleOnMouseOut(event) {
    if (document.querySelector('div.string-1.inactive') !== null)
      TweenMax.to(document.querySelector('div.string-1.inactive'), 0.5, { css: {opacity: 0.5} });
  }

  render() {
    return (
      <div>
        <div className={this.props.activeStringPart === "string-1" ? "string-1 active" : "string-1 inactive"}>
          <span className="mark-active-click"></span>
          <h3>Strand 1 Color</h3>
          {this.selectComponentToRender()}
        </div>
        <span
          id="string-1"
          className="string-1-click-area"
